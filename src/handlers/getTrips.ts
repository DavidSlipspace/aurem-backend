...
export async function handler(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  try {
    ...
    if (currentUser.roleName === "admin") {
    ...
    } else if (currentUser.roleName === "case_manager") {
      ...
      return jsonResponse(403, {
        message: "User role is not authorized to view trips."
      });
    }
    ...
    const result = await pool.query<TripRow>(
      ...
      JOIN gc_profiles gp ON gp.id = t.gc_profile_id
      WHERE ${whereClause}
    ...
    return jsonResponse(200, {
      trips: result.rows.map((row) => ({
        id: row.id,
        tripReferenceId: row.trip_reference_id,
        ...
        outboundAirport: row.outbound_airport,
        returnAirport: row.return_airport,
        destinationCity: row.destination_city,
        destinationAddress: row.destination_address,
        hotelProximityPreference: row.hotel_proximity_preference,
        minimumHotelStarRating: row.minimum_hotel_star_rating,
        budgetFilter: row.budget_filter,
        companionTraveler: row.companion_traveler
      }))
    });
  } catch (error) {
